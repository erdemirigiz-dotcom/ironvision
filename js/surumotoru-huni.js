"use strict";
/* ============================================================================
   SAHNE MOTORU (HUNİ TASLAK ÇATALI, 15.08.2026) — js/surumotoru.js'nin
   kopyası; sadece kuş rengi turuncudan logo-mavisine çevrildi (batColor +
   shadowColor). js/surumotoru.js (canlı sitede kullanılan) DOKUNULMADI.
   ============================================================================
...
window.SahneMotoru = (function () {
  ...
  // ---- Metin güvenli-bant sınırı (Demir revize 15.08: kuş yazısı başlıkla
  // ÇAKIŞMASIN) — hero-local piksel: "IRON VISION" harfleri bu Y'den AŞAĞI
  // asla taşmaz; site-huni.js her resize'da hero-alt'ın gerçek konumuna göre
  // günceller (bkz. metinSiniri API'si). null ise sınırsız (eski davranış).
  let formMaxBottom = null;
  ...
  // ---- Gövde rengi kademesi: soluk gece laciverti → parlak kobalt -----------
  // HUNİ TASLAK (15.08): logodaki "keskin göz" iris renginden örneklendi
  // (goz-etsy-ikon-500x500.png, PIL piksel okuma — parlak kobalt ort. rgb(10,83,211)).
  // Karanlık zeminde görünür kalması için lerp'in her iki ucu da orijinal
  // turuncu paletle aynı parlaklık aralığında tutuldu, sadece renk ailesi
  // turuncudan maviye çevrildi (Demir emri: "mavi aile — turuncu kalmasın").
  function batColor(t) {
    return "rgb(" + Math.round(lerp(92, 130, t)) + "," +
                    Math.round(lerp(112, 185, t)) + "," +
                    Math.round(lerp(156, 255, t)) + ")";
  }
  ...
  function buildSprites() {
    ...
    for (let fi = 0; fi < WING_FRAMES; fi++) {
      ...
      g.fillStyle = batColor(0.92);   // parlak kobalt (logo iris tonu)
      g.shadowColor = "#3d7bff";
  ...
  function buildTargets() {
    ...
    // Güvenli bant: metin YALNIZCA [topMargin, bottomBound] arasına kurulur.
    // bottomBound site-huni.js'ten gelir (hero-alt'ın hero-local üst kenarı,
    // boşluk payı düşülmüş) — hangi viewport'ta olursa olsun başlık bloğuyla
    // çakışma buradan matematiksel olarak imkansız hale gelir. Bant çok kısaldıysa
    // (alçak pencere) yazı KOMPLE gizlenir — sürü tamamen serbest kalır (Demir 15.08).
    const topMargin = Math.max(H * 0.02, 8);
    const bottomBound = (typeof formMaxBottom === "number") ? formMaxBottom : H;
    const availH = bottomBound - topMargin;
    const MIN_AVAIL = 56; // altına inince 2 satır "IRON VISION" okunaklı kalmaz — gizle
    if (availH < MIN_AVAIL) { targets = []; return; }

    const cacheKey = formText + "|" + formYFrac + "|" + W + "x" + H + "|" + bottomBound;
    ...
    if (!pts) {
      ...
      // Dikey ortalama YALNIZCA güvenli bant içinde — eski davranış (H*formYFrac)
      // sadece formMaxBottom hiç verilmediyse (bu taslakta her zaman verilir) devrede.
      const cy = (typeof formMaxBottom === "number") ? (topMargin + availH / 2) : (H * formYFrac);
      let fs = Math.min(H * 0.30, W * 0.30, availH / (lines.length * 1.02));
  ...
  function tick(now) {
    ...
    // ---- Gezinme soluklaşması (Demir revize 15.08): "hero'nun alt köşelerinde
    // başıboş nokta kalmasın, yazı bloğunun çevresinde derli toplu dursun".
    // Fiziği DEĞİŞTİRMEDEN (sınırları zorlamak "duvar/çerçeve" görünümü
    // yaratıyordu — denendi, denemede rasgele hep kenara yığıldılar), sadece
    // ÇİZİMDE: merkezden uzaklaşan serbest kuş yumuşakça SÖNER (opacity 0'a
    // iner). Böylece köşelere savrulan taneler görünmez olur, hero içinde
    // görünen sürü her zaman merkezde derli toplu kalır.
    const ROAM_CX = W * 0.5, ROAM_CY = H * 0.5;
    const ROAM_R = Math.min(W, H) * 0.44;
    const ROAM_BAND = Math.min(W, H) * 0.24;
    ...
    for (let i = 0; i < bats.length; i++) {
      ...
      if (b.mode === "form" && (now - formStart) > b.delay) {
        ...
        // Hero metin kutusu itmesi: slogan civarına yığılma. Demir revize
        // 15.08 ("sol kenarda dikey kuş sütunu"): eski hali sert bir eşikte
        // (pad=44) tam güçle itiyordu — kutunun hemen dışında itki aniden
        // SIFIRA düşünce hiçbir kuvvet onları uzaklaştırmıyordu, yüzlerce
        // serbest kuş kutunun kenarında ince bir "çerçeve" halinde donup
        // kalıyordu (metin kurulamadığında TÜM sürü serbest kaldığı için bu
        // çerçeve tam bir dikey/yatay sütun gibi görünüyordu). Şimdi itki
        // kutudan uzaklaştıkça bir ek `fade` bandı boyunca YUMUŞAKÇA sıfıra
        // iniyor — kuş kutunun kenarında asılı kalmıyor, doğal akışa dönüyor.
        if (koruma) {
          const pad = 44, fade = 170;
          const kx0 = koruma.x - pad - fade, kx1 = koruma.x + koruma.w + pad + fade;
          const ky0 = koruma.y - pad - fade, ky1 = koruma.y + koruma.h + pad + fade;
          if (b.x > kx0 && b.x < kx1 && b.y > ky0 && b.y < ky1) {
            const edgeDist = Math.max(
              koruma.x - pad - b.x, b.x - (koruma.x + koruma.w + pad),
              koruma.y - pad - b.y, b.y - (koruma.y + koruma.h + pad)
            );
            const k = clamp(1 - edgeDist / fade, 0, 1);
            if (k > 0) {
              const cxk = koruma.x + koruma.w / 2, cyk = koruma.y + koruma.h / 2;
              const dx = b.x - cxk, dy = b.y - cyk, d = Math.hypot(dx, dy) || 1;
              ax += dx / d * 0.85 * k; ay += dy / d * 0.85 * k;
            }
          }
        }
      ...
      if (b.mode === "form") {
        ...
        alpha *= b.fade;
        const rdx = b.x - ROAM_CX, rdy = b.y - ROAM_CY;
        const rdist = Math.sqrt(rdx * rdx + rdy * rdy);
        if (rdist > ROAM_R) alpha *= clamp(1 - (rdist - ROAM_R) / ROAM_BAND, 0, 1);
  ...
  return {
    ...
    // Metin güvenli-bant alt sınırı (hero-local px). site-huni.js her resize'da
    // hero-alt'ın gerçek üst kenarına göre günceller; "IRON VISION" harfleri
    // bu Y'nin ALTINA asla kurulmaz — çakışma kesin önlenir (Demir revize 15.08).
    metinSiniri: function (maxBottom) {
      formMaxBottom = (typeof maxBottom === "number") ? maxBottom : null;
      targetCache.clear();
      if (state === S.FORMING || state === S.LOCKED) { buildTargets(); assignTargets(); }
    },
    // Kuş yazısı gerçekten kuruldu mu? (Demir revize 15.08: başlık bloğu çok
    // uzarsa güvenli bant daralır, buildTargets() targets=[] döner — "IRON
    // VISION" hiç kurulmaz. site-huni.js bunu okuyup yetim kalan "TOOLS"
    // etiketini gizler; JS-dışı bir tahmine gerek kalmaz.)
    metinVarMi: function () { return targets.length > 0; },
